// /static/realgrid/realgrid-utils.js
...
function setContextMenu(grid) {
    ...
    const colText = (column && column.header) ? column.header.text : "선택 안됨";
    var contextMenu = [
        {
            label: "고정",
            children: [
...
export function onContextMenuClick(grid, data, index) {
...
export function applyEditableStyleCallback(columnList, editableFields = []) {
  editableFields.forEach(field => {
    const col = columnList.find(col => col.fieldName === field || col.name === field);
    if (col) {
      col.styleCallback = function (grid, dataCell) {
        const row = grid.getDataSource().getJsonRow(dataCell.index.dataRow);
        const ret = { styleName: "text-right" };
        if (!row?.isEditable) {
          ret.editable = false;
          ret.styleName += " readonly-cell";
        }
        return ret;
      };
    }
